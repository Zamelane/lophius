...
import { EditSectionItem } from '@/src/shared/ui/custom/custom-edit'
...
import type { LanguageInfoDataType } from '@/src/shared/types/edit-types'
...
export function CreateLanguageDialog({ children, setValue, languages }: Props) {
  ...
  function handleSave() {
    for (const language of languages.get)
      if (
        language.iso_639_1.toLocaleLowerCase() === iso_639_1.toLocaleLowerCase()
      ) {
    ...
    if (iso_639_1.length !== 2) {
      toast.warning('ISO-639-1 некорректен (┳◇┳)', {
        description:
          'Код ISO-639-1 должен состоять из 2х символов. Пожалуйста, поищите эти коды в интернете, если не уверены в правильности.'
      })
      return
    }
    ...
    const valueToSet = {
      iso_639_1,
      english_name:
        english_name.substring(0, 1).toUpperCase() +
        english_name.substring(1, english_name.length).toLocaleLowerCase(),
      translates: [
        ...(translations.filter(
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Введите код языка (ISO 639-1) и его переводы. Когда закончите,
