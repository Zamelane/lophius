...
export const verifySession = cache(async (): Promise<authData> => {
  ...
  if (!cookie) return noAuthObject

  const session = await decrypt(cookie)

  if (!session?.userId) return noAuthObject
...
export const getUserById = cache(
  async (userId: number): Promise<undefined | UserInfo> => {
    try {
...
export const getUserByNickname = cache(
  async (nickname: string): Promise<undefined | UserInfo> => {
    try {
      // const avatar = alias(files, 'avatar');
      // const background = alias(files, 'background');
      return db
