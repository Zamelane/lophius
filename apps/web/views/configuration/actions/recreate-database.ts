...
export async function RecreateDatabase(config: Config): Promise<ApiResponse<undefined>> {
  console.log(55555)
  try {
    ...
    await adminClient.query(`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'lophius' AND pid <> pg_backend_pid();`)
