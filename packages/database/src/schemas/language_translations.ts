...
export const language_translations = pgTable('language_translations', {
  ...
  languageId: integer()
    ...
    .notNull(), // Для какого языка перевод
  translateValueLanguageId: integer()
    .references(() => languages.id)
    .notNull() // Сам перевод
})

export type LanguageTranslationsTableType =
  typeof language_translations.$inferSelect

export const languageTranslationsRelations = relations(
