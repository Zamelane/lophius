...
import { SimpleTitle } from '../../shared/ui/typography/simple-title'
//import { ProfileActivityChart } from "./charts/activity-chart"

export const ProfileInfo = ({ data }: { data: UserInfo }) => {
  const stats = [
    ...
    <div className='flex flex-col flex-grow gap-3 py-2 max-w-full'>
      <SimpleTitle title='О себе'>
        <i>
          {data.bio?.length
            ? data.bio
            : 'Пользователь ничего нам не рассказал...'}
        </i>
      </SimpleTitle>
      ...
      <SimpleTitle title='Статистика'>
        <div className='flex flex-grow flex-wrap gap-3'>
          {stats.map((v, i) => (
            <div
              key={i}
              className='flex flex-col gap-1 flex-grow border-[1px] border-border px-3.5 py-3 rounded-sm'
            >
              <p className='text-sm font-semibold opacity-80 text-nowrap'>
                {v.title}
              </p>
