...
type Props = WithOptional<List, 'id' | 'isSystem'> &
  LayoutProps & {
    isEdit?: boolean
          ...
          <div className='flex flex-col gap-4'>
            <SheetHeader>
              <SheetTitle className='text-xl'>
                {isEdit ? 'Редактирование списка' : 'Новый список'}
              </SheetTitle>
            </SheetHeader>
            ...
            {isSystem && (
              <div className='flex gap-4 items-center relative w-full rounded-lg border p-4 [&>svg~*]:pl-7[&>svg]:text-foreground bg-blue-500/30 border-none text-foreground'>
                <p className='text-sm'>
                  У системных списков нельзя изменить имя. Это нужно для более
                  точного сбора статистики.
                </p>
              </div>
            )}
            ...
            <div className='grid w-full items-center gap-1.5'>
              ...
              >
                Имя
                {!isSystem && (
                  <p className='text-base text-red-500 leading-3'>*</p>
                )}
              </Label>
              <Input
                type='text'
            ...
            <div className='items-top flex space-x-2 py-1'>
              <Checkbox
                id='isHidden'
                checked={isHidden}
                onCheckedChange={(checked) => setIsHidden(Boolean(checked))}
              />
              <div className='grid gap-1.5 leading-none'>
                <Label htmlFor='isHidden'>Скрытый список ?</Label>
                <p className='text-sm text-muted-foreground'>
                  О содержимом списка и его наличии будете знать только вы.
                </p>
              </div>
            </div>
          </div>
