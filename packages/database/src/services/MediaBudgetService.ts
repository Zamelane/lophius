...
import {db, Transaction, DBConnection, MediaBudgetRepository, MediaBudget} from "../index";
...
export class MediaBudgetService extends BaseService {
	...
	saveBudget(media: MediaModel, budget: MediaBudget['budget'] | undefined) {
		if (budget) {
			...
			this.uow.registerOperation('insert', this.mediaBudgetRepository, mediaBudgetModel, media)
