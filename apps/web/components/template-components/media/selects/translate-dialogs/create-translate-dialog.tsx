...
import { LanguageInfoDataType, KinoTranslateInfoDataType } from "@/interfaces/edit-types"
...
export function CreateTranslateDialog({ children, languages, translates }: Props) {
  ...
  function add() {
    ...
    if (translates.get.find(v => v.language.iso_639_1 === value.iso_639_1)) {
      toast.warning(`Выбранный выами язык (${value.iso_639_1}) уже добавлен`, {
        description: 'Пожалуйста, воспользуйтесь поиском, ' +
        'чтобы добавить перевод для желаемого вами языка, т.к. он уже добавлен.'
      })
      return
    }

    translates.set(state => [...state, {
      overview: "",
      titles: [""],
      language: value
    }])
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Выберите язык, для которого хотите добавить перевод или альтернативные заголовки.
          </DialogDescription>
