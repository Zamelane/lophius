import type { SourceId, WithRequired } from '../../index'
import type { OptionalExternalImage, OptionalExternalImageId } from './type'
...
export class ExternalImageModel implements OptionalExternalImage {
  ...
  validateRequiredModels(): asserts this is WithRequired<
    OptionalExternalImage,
    'externalDomain' | 'externalDomainId'
  > {
  ...
  > {
    ...
    if (!this.id || !this.externalDomainId)
      throw new Error(
        `Missing required id and sourceId: ${this.constructor.name}${JSON.stringify(
          [this.id, this.sourceId, this.externalDomainId]
        )}`
      )
  }
}
