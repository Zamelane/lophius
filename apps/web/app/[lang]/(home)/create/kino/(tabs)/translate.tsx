...
export function TranslateTab({
  ...
  function setCollapsed(index: number) {
    setCollapsied((state) =>
      state
        .slice()
        .map((v, i) => (i === index ? { isCollapsed: !v.isCollapsed } : v))
    )
  }
    ...
    if (!titles.length || titles[titles.length - 1].length > 1) {
      setTranslates((state) => {
        return state.map((item, i) =>
          i === index ? { ...item, titles: [...item.titles, ''] } : item
        )
      })
    }
  }
    ...
    setTranslates((state) =>
      state.map((v, i) =>
        i === index
          ? {
              ...v,
              titles: v.titles.map((t, ti) => (ti === titleIndex ? value : t))
            }
          : v
      )
    )
  ...
  function overviewChange(index: number, value: string) {
    setTranslates((state) =>
      state.map((v, i) => (i === index ? { ...v, overview: value } : v))
    )
  }
          ...
          (v, i) =>
            (!search.length ||
              ...
              v.language.iso_639_1
                .toLocaleLowerCase()
                .includes(search.toLocaleLowerCase()) ||
              (v.language.translates?.filter((v) =>
                v.value.toLocaleLowerCase().includes(search.toLocaleLowerCase())
              )?.length ?? 0) > 0) && (
                      ...
                      <div className='flex flex-row items-center'>
                        <CardTitle className='flex gap-1 items-items-start'>
                          ...
                          {!v.titles[0].trim().length &&
                            !v.overview.trim().length && <BookDashedIcon />}
                        </CardTitle>
                      </div>
                      <CardDescription className='px-1'>
                        {(collapsied.length - 1 >= i ||
                          collapsied.push({ isCollapsed: true })) &&
                          collapsied[i].isCollapsed && (
                            <div>
                              {v.titles[0].length > 0 && (
                                <div className='grid grid-cols-[auto,1fr] items-center'>
                                  <TypeIcon width={16} />
                                  <p className='text-md italic text-muted-foreground truncate'>
                                    {v.titles[0]}
                                  </p>
                                </div>
                              )}
                        ...
                        {v.titles.map((vt, ti) => (
                          <EditSectionGroup key={`t_${ti}`}>
                            <Input
                              value={vt}
                              onChange={(v) =>
                                titleChange(i, ti, v.target.value)
                              }
                              placeholder={
                                ti === 0
                                  ? 'Основной перевод названия'
                                  : 'Альтернативный перевод названия'
                              }
                            />
                            {ti !== 0 && (
                              <Button
