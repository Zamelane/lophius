...
export async function findMediaById(
  ...
  return queryOneResult(
    await this.tx.select().from(medias).where(eq(medias.id, mediaId))
  )
}
...
export async function findMediaByExternalId(
  ...
  const r = queryOneResult(
    await this.tx
      ...
      .from(medias)
      .where(
        and(eq(medias.external_id, externalId), eq(medias.sourceId, sourceId))
      )
  )
...
export async function updateMedia(this: MediaRepository, media: MediaModel) {
