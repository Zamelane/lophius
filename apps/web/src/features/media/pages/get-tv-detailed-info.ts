...
export async function getTvDetailedInfo({
  id,
  alreadyLoadedData
}: Props): Promise<GetTvDetailedInfoResult | undefined> {
  ...
  return {
    ...
    translates: {
      runtimes: translates.filter((v) => v.runtime > 0).map((v) => v.runtime),
      titles: translates
    ...
    backdrops: (query_result.externalBackdrops ?? [])
      .map((v) => {
