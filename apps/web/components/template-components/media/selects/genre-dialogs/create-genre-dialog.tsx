...
import type { Language, Translate, WithRequired } from '@/interfaces'
import type {
  GenreInfoDataType,
...
export function CreateGenreDialog({
  ...
  function add() {
    if (
      translations.length &&
      translations[translations.length - 1].value.trim().length < 1
    )
      return
  ...
  function handleSave() {
    for (const genre of genres.get)
      if (
        genre.english_name.toLocaleLowerCase() ===
    ...
    if (englishName.length < 4) {
      toast.warning('Жанр некорректен (┳◇┳)', {
        description:
          'Английское название жанра должно состоять хотя бы из 4х символов. Пожалуйста, поищите првильные название в интернете, если не уверены в правильности.'
      })
      return
    }
    ...
    const valueToSet = {
      ...
      english_name:
        englishName.substring(0, 1).toUpperCase() +
        englishName.substring(1, englishName.length).toLocaleLowerCase(),
      translates: [
        ...(translations.filter(
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Введите жанр на английском и его переводы. Когда закончите,
        ...
        <div className='grid gap-4'>
          <div className='flex flex-row items-center gap-4'>
            <Input
              ...
              className='w-full'
              value={englishName}
              placeholder='Английское именование'
              onChange={(v) => englishNameInputChange(v.target.value)}
            />
          </div>
        </div>
            ...
            <div key={`tc_${i}`} className='flex flex-row gap-4'>
              ...
              <div className='w-full flex flex-row gap-2'>
                <Input
                  value={v.value}
                  id={`tc_i_${1}`}
                  placeholder='Перевод на другой язык'
